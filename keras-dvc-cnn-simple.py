...
parser = argparse.ArgumentParser(description='Dogs-us-cats with keras.')
parser.add_argument('--data_path', type=str, default='/valohai/inputs/')
parser.add_argument('--results_path', type=str, default='/valohai/output/')
parser.add_argument('--log_path', type=str, default='valohai/output/')
...
if K.backend() == "tensorflow":
    ...
    import os, datetime
    logdir = os.path.join(args.log_path, "logs",
                     "dvc-simple-"+datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    print('TensorBoard log directory:', args.log_path)
    try:
        os.makedirs(logdir)
